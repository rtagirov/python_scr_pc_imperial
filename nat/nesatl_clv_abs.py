...
for band in bands:
    ...
    ax1.plot(mu, its_n / its_a, color = col[idx_b], label = l)
...
ax1.set_ylabel(r'$I_\mathrm{NESSY}(\mu) / I_\mathrm{ATLAS}(\mu)$')
...
leg = ax1.legend(frameon = True, framealpha = 1, loc = (axbox.x0 - 0.055, axbox.y0 + 0.15), prop={'size': 23})

for handle in leg.legendHandles: handle.set_linewidth(5.0)

pltaux.savepdf('nesatl_clv_abs')
