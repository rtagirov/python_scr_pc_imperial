...
for k in range(len(tau)):
    ...
    f = open('outputfile_tau_bot_' + str(tau[k]), 'r')
...
#    j = random.choice(np.arange(0, 512))
    ...
    for k in range(len(tau)):
        ...
        ax[0].plot(hc[k, i, :], Tc[k, i, :], label = 'interpolated (' + r'$\tau_{500}^\mathrm{bot}$ = ' + str(tau[k]) + ')')
        ax[1].plot(hc[k, i, :], pc[k, i, :])
        ax[2].plot(hc[k, i, :], dc[k, i, :])
    ...
    ax[0].text(0.12, 0.95, r'$\tau_{500}^\mathrm{top}$ = 1.0e-9', transform=ax[0].transAxes, fontsize=10,
               verticalalignment='top', bbox=props)
...
os.system('pdftk ' + pdfs + ' output atms_bot.pdf')
