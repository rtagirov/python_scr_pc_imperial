...
ax[0].plot(w, qnf, color = 'g', label = 'NESSY - FAL', linewidth = 3)
ax[0].plot(w, qnk, color = 'r', label = 'NESSY - KUR')
ax[0].plot(w, qlk, color = 'b', label = 'ATLAS - KUR')

ax[1].plot(w, fnf, color = 'r', label = 'NESSY - FAL')
ax[1].plot(w, flk, color = 'b', label = 'ATLAS - KUR')

ax[2].plot(w, pnk, color = 'r', label = 'NESSY - KUR')
ax[2].plot(w, plk, color = 'b', label = 'ATLAS - KUR')

ax[3].plot(w, unk, color = 'r', label = 'NESSY - KUR')
ax[3].plot(w, ulk, color = 'b', label = 'ATLAS - KUR')
...
for i in range(len(ax)):
    ...
    if i != 3: leg = ax[i].legend(framealpha = 1, loc = 1, handletextpad = 1, prop = {'size': 15.5})
    if i == 3: leg = ax[i].legend(framealpha = 1, loc = 4, handletextpad = 1, prop = {'size': 15.5})

    for obj in leg.legendHandles: obj.set_linewidth(3.0)

auxplt.savepdf('var/inp_spec_lin')
