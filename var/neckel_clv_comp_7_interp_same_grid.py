...
int_n0_sg = np.zeros((len(wvl_at), len(mu)))
int_n1_sg = np.zeros((len(wvl_at), len(mu)))
int_n2_sg = np.zeros((len(wvl_at), len(mu)))

for i in range(len(mu)):

    int_n0_sg[:, i] = spec.mean_over_grid(int_n0[:, i], wvl_n0, wvl_at)
    int_n1_sg[:, i] = spec.mean_over_grid(int_n1[:, i], wvl_n1, wvl_at)
    int_n2_sg[:, i] = spec.mean_over_grid(int_n2[:, i], wvl_n2, wvl_at)
...
#    nes_pairs.append(wvl_ne0)
    ...
    clv_ne0 = interp_clv(idx_atl, wvl_atl, int_n0_sg, w[i])
    clv_ne1 = interp_clv(idx_atl, wvl_atl, int_n1_sg, w[i])
    clv_ne2 = interp_clv(idx_atl, wvl_atl, int_n2_sg, w[i])
...
ax[0].text(1.0 - shift,   19.8, 'ATLAS9, LTE, U99', fontsize = 10)
ax[0].text(0.775 - shift, 19.8, 'NESSY, LTE, U99', color = 'm', fontsize = 10)
ax[0].text(0.565 - shift, 19.8, 'NESSY, NLTE, U99', color = 'g', fontsize = 10)
ax[0].text(0.335 - shift, 19.8, 'NESSY, NLTE, FAL99', color = 'r', fontsize = 10)

auxplt.savepdf('var/neckel_clv_comp_7_interp_od_same_grid')
