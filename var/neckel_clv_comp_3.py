...
f1 = paths.it0f + 'var/Q/kur/CLV_UVI'
f2 = paths.it1f + 'var/Q/kur/CLV_UVI'
f3 = paths.it1f + 'var/Q/fal/CLV_UVI'
...
for j, f in enumerate([f1, f2, f3]):
    ...
    ax[j].set_xlim(1.0, 0.0)

    ax[j].set_xlabel(r'$\mu$')

    for i in range(len(w) - 1, -1, -1):

        ax[j].plot(mu, P(A[i, :], mu), label = str(w[i]) + ' nm', color = col[i])

        clv_nessy = int_n[idx_n[i], :] / int_n[idx_n[i], 0]
        clv_atlas = int_a[idx_a[i], :] / int_a[idx_a[i], 0]

        ax[j].scatter(mu, clv_nessy, s = ss * 2, marker = 'o', facecolors = 'none', edgecolors = col[i])
        ax[j].scatter(mu, clv_atlas, s = ss, marker = '*', color = col[i])

leg = ax[0].legend(framealpha = 1, loc = 3, handletextpad = 1, prop = {'size': 17.0})

for obj in leg.legendHandles: obj.set_linewidth(3.0)

auxplt.savepdf('var/neckel_clv_comp_3')
