...
RDPle30 = (PC30 - PCle) * 100.0 / PCle
RDSle30 = (SC30 - SCle) * 100.0 / SCle
...
ax[0].plot(w, PCle, color = 'r', linewidth = lw, linestyle = ls, label = 'Facular (LTE)')
ax[0].plot(w, SCle, color = 'b', linewidth = lw, linestyle = ls, label = 'Spot (LTE)')

ax[0].plot(w, PC30, color = 'r', linewidth = lw, label = 'Facular (NLTE)')
ax[0].plot(w, SC30, color = 'b', linewidth = lw, label = 'Spot (NLTE)')
...
ax[1].plot(w, abs(RDSle30), color = 'b', linewidth = lw)
ax[1].plot(w, abs(RDPle30), color = 'r', linewidth = lw)
...
ax[0].set_ylabel('Contrast, [W / m$^2$ / nm]',                   fontsize = 12.5)
ax[1].set_ylabel('Relative difference: |NLTE - LTE| / LTE, [%]', fontsize = 12.5)

leg0 = ax[0].legend(framealpha = 1, loc = 4, handletextpad = 1, prop = {'size': 18.5})
...
#auxplt.savepdf('contr_nltevslte_' + smoothing)
auxplt.savepdf('contr_nltevslte')
