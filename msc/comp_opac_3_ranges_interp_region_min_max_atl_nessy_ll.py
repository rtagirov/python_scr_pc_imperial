...
dir_n1 = '/mnt/SSD/sim/runs/hminus/IT1/atlodf/new/fal/nopre_noH_nopif/'
#dir_n2 = '/mnt/SSD/sim/runs/hminus/IT1/atlodf/new/fal/nopre_noH_nopif_nessy_ll/'
dir_n2 = '/mnt/SSD/sim/runs/hminus/IT1/atlodf/new/fal/nopre_noH_nopif_nessy_ll_ab/'
#dir_ol = '/mnt/SSD/sim/runs/hminus/IT1/atlodf/old/fal/base_mol/lbkg/'
dir_ol = '/mnt/SSD/sim/runs/hminus/IT1/runtime/def/lbkg/'
dir_oc = '/mnt/SSD/sim/runs/hminus/IT1/atlodf/old/fal/base_mol/lbkg_cont/'

file_n1 = dir_n1 + 'linop.out'
file_n2 = dir_n2 + 'linop.out'

T =  np.loadtxt(dir_n1 + 'atm.inp', usecols = [1])
n =  np.loadtxt(dir_n1 + 'atm.inp', usecols = [3])

ne = np.loadtxt(dir_n1 + 'inine.out') * n

tmdp = np.argmin(T) + 1

ndp = auxfile.num_lines(dir_n1 + 'atm.inp')

nwv = auxfile.num_lines(file_n1)

wvl_o = np.arange(1000, 10000, 10)

opa_ol = np.zeros((len(wvl_o), ndp))
opa_oc = np.zeros((len(wvl_o), ndp))

for i, wvl in enumerate(wvl_o):

    opa_ol[i, :] = np.loadtxt(dir_ol + str(wvl + 5) + '.lbkg', skiprows = 1)
    opa_oc[i, :] = np.loadtxt(dir_oc + str(wvl + 5) + '.lbkg', skiprows = 1)

lam_n =  np.loadtxt(file_n1, usecols = [0])
kap_n1 = np.loadtxt(file_n1, usecols = [4])
kap_n2 = np.loadtxt(file_n2, usecols = [4])

nwv = int(nwv / ndp)

wvl_n = np.zeros(nwv)
opa_n1 = kap_n1.reshape((nwv, ndp))
opa_n2 = kap_n2.reshape((nwv, ndp))
...
#        ax[j].step(wvl_o / 10.0, opa_ol[:, i],                label = 'NESSY (sorted and averaged, lines)',            where = 'post', color = 'k')



        ax[j].step(wvl_o / 10.0, opa_ol[:, i],                label = 'NESSY (original)',            where = 'post', color = 'k')
        ax[j].step(wvl_o / 10.0, opa_oc[:, i],                label = 'NESSY (sorted and averaged, continuum)', where = 'post', color = 'gray')
#        ax[j].step(wv_u,         apm * n[i] * op_u[:, k],     label = 'ATLAS (sorted and averaged)', where = 'post', color = 'purple', linewidth = 1.5)
#        ax[j].step(wvl_n / 10.0, opa_n1[:, i],                label = 'NESSY (interpolation, lines, ATLAS linelist)',  where = 'post', color = 'g',      linewidth = 0.5)
#        ax[j].step(wvl_n / 10.0, opa_n2[:, i],                label = 'NESSY (interpolation, lines, NESSY linelist)',  where = 'post', color = 'r',      linewidth = 0.5)

        ax[j].step(wvl_n / 10.0, opa_n2[:, i],                label = 'NESSY (interpolated)',  where = 'post', color = 'r',      linewidth = 0.5)
