...
op1_v = 10.0**(np.loadtxt(paths.inp + 'interpolated_200_250new.dat', usecols = [3]) / 1000.0)
op2_v = 10.0**(np.loadtxt(paths.inp + 'interpolated_590_670new.dat', usecols = [3]) / 1000.0)
op3_v = 10.0**(np.loadtxt(paths.inp + 'interpolated_800_850new.dat', usecols = [3]) / 1000.0)
...
xmin = np.array([200, 590, 800])
xmax = np.array([260, 670, 860])
...
#        ax[j].fill_between(otw, odf_min[i, :],                       odf_max[i, :],               step = 'post', facecolor = 'blue', label = 'new', edgecolor = 'blue')
#        ax[j].fill_between(otw, apm * n[i] * odf[:, ip - 1, it - 1], apm * n[i] * odf[:, ip, it], step = 'post', facecolor = 'red',  label = 'old', edgecolor = 'red')
        ax[j].fill_between(otw, odf_min[i, :],                       odf_max[i, :],               step = 'post', facecolor = 'grey', label = 'odf table', edgecolor = 'grey')

        ax[j].step(wvl_o / 10.0, opa_o[:, i],     label = 'NESSY (sorted and averaged)', where = 'post', color = 'k')

        if i == 54 and j == 1:

            ax[1].step(wv2_u, apm * n[i] * op2_u, label = 'ATLAS (sorted and averaged)', where = 'post', color = 'purple', linewidth = 0.8)
    ...
    if i == 54:
        ...
        leg1 = ax[1].legend(framealpha = 1, loc = 2, handletextpad = 1, prop = {'size': 7.5})
