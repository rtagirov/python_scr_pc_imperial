...
dir_o1 = '/mnt/SSD/sim/runs/hminus/IT1/atlodf/old/fal/base_mol/lbkg/'
dir_o2 = '/mnt/SSD/sim/runs/hminus/IT1/atlodf/old/fal/base_no_mol/lbkg/'
...
opa_o1 = np.zeros((len(wvl_o), ndp))
opa_o2 = np.zeros((len(wvl_o), ndp))

for i, wvl in enumerate(wvl_o):

    opa_o1[i, :] = np.loadtxt(dir_o1 + str(wvl + 5) + '.lbkg', skiprows = 1)
    opa_o2[i, :] = np.loadtxt(dir_o2 + str(wvl + 5) + '.lbkg', skiprows = 1)
...
#        ax[j].fill_between(otw, odf_min[i, :], odf_max[i, :], label = 'odf table', step = 'post', facecolor = 'grey', edgecolor = 'grey')
        ...
        ax[j].step(wvl_o / 10.0, opa_o1[:, i],                 label = 'NESSY (sorted and averaged, molecules)',    where = 'post', color = 'k')
        ax[j].step(wvl_o / 10.0, opa_o2[:, i],                 label = 'NESSY (sorted and averaged, no molecules)', where = 'post', color = 'r', linewidth = 0.5)
