...
wvl = np.loadtxt(paths.it0h + 'test_grid/xjc.out', skiprows = 1948 * 81, usecols = [2]) / 10.0

#xjc_1d = np.loadtxt(paths.it0h + 'test_grid/xjc.out', usecols = [3])

sig_1d = np.loadtxt(paths.it0h + 'test_grid/sigma.out', usecols = [3])
...
sysaux.clean_dir(paths.figdir + 'cross_section', mode = 'noverbose')
...
for i in tqdm(range(1, 31), ncols = auxfunc.term_width(), desc = 'Plotting'):
    ...
    for m in range(0, len(wvl)):

        if np.logical_not(np.isnan(sig[elem[len(elem) - 1] - 1, m])): ax.axvline(wvl[m], color = 'k', linewidth = 0.05)
    ...
    pltaux.savepdf('cross_section/' + str(i))

os.chdir(paths.figdir + 'cross_section/')

os.system('pdftk ' + names_pdf + 'output ' + 'cs.pdf')
